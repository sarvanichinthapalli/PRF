...
class Network():
    ...
    def cluster_weights_agglo(self,weight, threshold,prunes,folder_name,conv_layer,prune_count, first=False, average=True):
        ...
        #print(cluster_assignments, len(cluster_assignments),len(np.unique(cluster_assignments)))
        ...
        if(first==False): # retain ONLY 1 from each cluster one shot
            ...
            for i in range(len(filters)):
               temp=[]
               for f in filters[i]:
                 temp.append(norm(weight[f],2)) #L1-norm#chaneddddddddddddddddddddddd
            ...
            for j in range(len(filters)):
                ...
                retain_filters= filters[j][cut:]
                ele.extend(retain_filters)'''    
                retain_filters= filters[j][-1]
                ele.append(retain_filters)
